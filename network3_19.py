...
save_dir = 'final_model_19/'
if not os.path.exists(save_dir):
    os.makedirs(save_dir)
save_path = os.path.join(save_dir, 'best_model_19')
...
with tf.Session() as sess:
	...
	for epoch in range(hm_epochs):
		...
		for g in range(0,len(X_train),batch_size):
			...
			epoch_loss += c
		print('Epoch', epoch, '/',hm_epochs,'loss :',epoch_loss,'learning rate :',l_rate)
		if epoch_loss > prec:
		  compteur += 1
		else:
		  if compteur > 0:
		    compteur -= 1
		  prec = epoch_loss
		  print('Accuracy Test :',accuracy.eval({keep_prob: 1, x:X_test[:2048], y:y_test[:2048]}))
