...
def recup():
  ...
  for num,noms in enumerate(['s_bord1','s_bord2','s_bord3','s_bord4','s_centre','s_coin1',\
    ...
    if num < 10:
      ...
      classe[1] = 1
    compteur = 0
    while ligne != "" and compteur <= 10000:
  ...
  lenX = int(len(recupe)*0.9)
  return np.array(list(recupe[:lenX,0]), dtype='float32'), \
		 np.array(list(recupe[lenX:,0]), dtype='float32'), \
		 np.array(list(recupe[:lenX,1]), dtype='float32'), \
		 np.array(list(recupe[lenX:,1]), dtype='float32')
...
X_train, X_test, y_train, y_test = recup()
lenX = len(X_train)
leni = len(X_test)
print(lenX,leni)
X_recup, y_recup = 0, 0
...
layer_fc2, weights_fc = new_fc_layer(input=layer_flat_drop,
                         num_inputs=num_features,
...
l_rate = 0.001#5e-4
beta = 0.01
cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(layer_fc2,y) + beta * (tf.nn.l2_loss(weights_conv1) + tf.nn.l2_loss(weights_fc)))
...
with tf.Session() as sess:
	...
	while res < 0.999:
		...
		for g in range(0,len(X_train),batch_size):
			...
			epoch_loss += c
		print('Epoch', epoch,'loss :',epoch_loss,'test set :', res,'learning rate :',l_rate)
		if epoch_loss > prec:
		  compteur += 1
		else:
		  if compteur > 0:
		    compteur -= 1
		  prec = epoch_loss
		  res = accuracy.eval({keep_prob: 1, x:X_test[:4096], y:y_test[:4096]})
	...
	t0 = t.time()
	c, res = 0, 0
	for g in range(0,len(X_train),4096):
		res += accuracy.eval({keep_prob: 1, x:X_train[g:g+4096], y:y_train[g:g+4096]})
		c += 1
	print('Accuracy Train :',res/c,'sur',lenX,'images en', t.time() - t0,'sec')

	t0 = t.time()
	c, res = 0, 0
	for g in range(0,len(X_test),4096):
		res += accuracy.eval({keep_prob: 1, x:X_test[g:g+4096], y:y_test[g:g+4096]})
		c += 1
